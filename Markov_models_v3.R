...
markovfunc<-function(iter=iter){
  ...
  for (a in 1:iter){
    ...
    for (j in 1:4){
      ...
      sim.mat[1,1]<-fome1.conc[a]*SA.fome.1[a] #starting # of viruses on fome 1, 100 per cm2
      sim.mat[2,1]<-fome2.conc[a]*SA.fome.2[a] #starting # of viruses on fome 2, 5 per cm2
