...
discretefunc<-function(iter=iter){
  ...
  for(a in 1:iter){
    ...
    for (j in 1:4){
      ...
      fome1[1]<-fome1.conc[a] #100/cm2
      fome2<-rep(NA,duration+1)
      fome2[1]<-fome2.conc[a] #5/cm2
    ...
    maxdoses<-c(max(dosetotal[j.save==1]),max(dosetotal[j.save==2]),max(dosetotal[j.save==3]),max(dosetotal[j.save==4]))
    ...
    param<-data.frame(fome1conc=rep(fome1.conc[a],4),
                      fome2conc=rep(fome2.conc[a],4),
                      dose=maxdoses)
